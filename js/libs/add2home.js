...
(function(){
	...
	options = {
		...
		lifespan: 50000,			// 20 seconds before it is automatically destroyed
...
if (hasHomescreen && !expired && !isStandalone && isSafari && !options.disableLoading) {
	...
	//window.addEventListener('load', onStandaloneLoad, false);
	onStandaloneLoad();
}

function onStandaloneLoad(){
	$.app.addToHomeClosed();
}
